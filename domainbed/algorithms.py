...
from domainbed import distributions
...
class ProbDG(Algorithm):
    """
    Probabilistic Domain Generaltization
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ProbDG, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
    ...
    def risk(self, x, y):
        return F.cross_entropy(self.network(x), y)

    def update(self, minibatches, unlabeled=None):
        beta = self.hparams['prob_dg_beta']
        
        for _ in range(self.hparams['prob_dg_n_steps']):

            # each element corresponds to the risk on a different environment
            env_risks = torch.tensor([self.risk(x, y) for x,y in minibatches])

            # indicator of whether each risk was larger or smaller than t
            indicators = torch.where(
                env_risks > self.t, 
                torch.ones_like(env_risks), 
                torch.zeros_like(env_risks))

            # update t via gradient descent
            grad_t = 1 - (1 / (1 - beta)) * indicators.mean()
            self.t = self.t - self.hparams['prob_dg_step_size'] * grad_t

        self.optimizer.zero_grad()

        env_risks = [F.relu(self.risk(x, y) - self.t) for x,y in minibatches]
        loss = (1 / (1-beta)) * sum(env_risks) / len(env_risks)
        loss.backward()
        self.optimizer.step()

        return {'loss': loss.item(), 'cvar': loss.item() + self.t.item()}


class QRMBase(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams, dist):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.register_buffer('update_count', torch.tensor([0]))
        self.register_buffer('q', torch.tensor(self.hparams["var_quantile"], dtype=torch.float64))
        self.dist = dist
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.update_count < self.hparams["qrm_burnin_iters"]:
            obj = torch.mean(env_risks)  # Burn-in period to discover features (=ERM if same num of samples in each env)
        else:
        ...
        return {'loss': obj.item()}


class QRM_Normal(QRMBase):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(QRM_Normal, self).__init__(input_shape, num_classes, num_domains, hparams, 
                                         dist=distributions.Normal())


class QRM_KDE(QRMBase):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(QRM_KDE, self).__init__(input_shape, num_classes, num_domains, hparams, 
                                      dist=distributions.Nonparametric(use_kde=True))


class QRM_KDE_silverman(QRMBase):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams,
                         dist=distributions.Nonparametric(use_kde=True,
                                                          bandwidth_infer_method='silverman'))
