...
def _hparams(algorithm, dataset, random_seed):
    ...
    """
    SMALL_IMAGES = ['Debug28', 'RotatedMNIST', 'ColoredMNIST', 'ColoredMNISTMultiTest', 'ColoredMNISTMultiTest2']
    ...
    elif algorithm == 'ProbDG':
        _hparam('prob_dg_n_steps', 10, lambda r: int(r.choice(list(range(5, 21)))))
        # _hparam('prob_dg_beta', 0.5, lambda r: r.uniform(0.01, 0.99))
        _hparam('prob_dg_beta', 0.90, lambda r: 0.90)
        _hparam('prob_dg_step_size', 0.1, lambda r: r.uniform(0.001, 0.5))

    elif 'QRM_' in algorithm:
        _hparam('var_quantile', 0.75, lambda r: r.uniform(0.5, 0.99))
        _hparam('qrm_burnin_iters', 1000, lambda r: 10*r.uniform(2, 3.5))
        _hparam('qrm_lr', 1e-6, lambda r: 10**r.uniform(-7, -5))
    ...
    if dataset in SMALL_IMAGES:
        _hparam('batch_size', 256, lambda r: int(2**r.uniform(6, 9)))
    elif algorithm == 'ARM':
