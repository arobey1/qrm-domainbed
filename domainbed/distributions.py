...
from domainbed.dist_utils import *
...
class Nonparametric(Distribution1D):
    def __init__(self, use_kde=False, bandwidth=0.5, bandwidth_infer_method='Gauss-optimal', adaptive_bandwidth=False):
        self.use_kde = use_kde
        self.bandwidth = bandwidth
        self.bandwidth_infer_method = bandwidth_infer_method
        self.data = None
        self.kde = None
        ...
        valid_b_methods = [None, 'silverman', 'median', 'median-HL', 'Gauss-optimal']
        if bandwidth_infer_method not in valid_b_methods:
            raise ValueError(f"Invalid method for inferring a good bandwidth. Choose one of {valid_b_methods}.")
    ...
    def estimate_parameters(self, x):
        ...
        alpha = 0.5 if self.adaptive_bandwidth else 0

        if self.use_kde:
            if self.bandwidth_infer_method is not None:
                self.kde = KernelDensityEstimator(self.data, bandwidth_est_method=self.bandwidth_infer_method,
                                                  alpha=alpha)
                self.bandwidth = self.kde.kernel.bandwidth
            else:
                self.kde = KernelDensityEstimator(self.data, self.kernel(self.bandwidth))
            if not self.kde.adaptive:  # not using adaptive bandwidth estimation
                self.bandwidth = torch.ones(1, device=self.data.device) * self.bandwidth

    def icdf(self, q):
        if self.use_kde:
            if q < 0:
                # looking for extreme values in right tail --> only need to consider the Gaussian with largest variance
                # Since all variances (i.e. bandwidths) are equal... TODO: check this means we only need rightmost Gauss
                # q actually represents log(y), where q = 1 - y is the quantile (avoids numerical issues)
                log_y = q
                v = torch.mean(self.data + self.bandwidth * math.sqrt(-2 * log_y))
                # print(v)
                return v

            else:
                # Find quantile via binary search on CDF
                lo = torch.distributions.Normal(self.data[0], self.bandwidth[0]).icdf(q)
                hi = torch.distributions.Normal(self.data[-1], self.bandwidth[-1]).icdf(q)

                return continuous_bisect_fun_left(self.kde.cdf, q, lo, hi)
        else:
            if q < 0:
                raise ValueError("Cannot provide log(1-q) unless using KDE.")
